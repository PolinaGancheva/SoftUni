function solve(input){
    let fires = input[0].split('#');
    let water = Number(input[1]);
    let validCells = [];
    let totalEffort = 0;
    let totalFire = 0;
    fires.forEach((fireLine) => {
        let [typeOfFire, valueOfCell] = fireLine.split(' = ');
        valueOfCell = Number(valueOfCell);
        let isValid = isFireValid(typeOfFire,valueOfCell);
        if(isValid && water >= valueOfCell){
            water -= valueOfCell;
            totalEffort += 0.25 * valueOfCell;
            totalFire += valueOfCell;
            validCells.push(valueOfCell);
        }
    });
    function isFireValid(type,value){
        if(type === 'High'&& value >= 81 && value <= 125){
            return true;
        }
         if(type === 'Medium' && value >= 51 && value <= 80){
             return true;
         }
          if(type === 'Low' && value >= 1 && value <= 50){
            return true;
        }
        return false;
    }
console.log('Cells:');
validCells.forEach((cell) => {
    console.log(` - ${cell}`);
});
console.log(`Effort: ${totalEffort.toFixed(2)}`);
console.log(`Total Fire: ${totalFire}`);
}
solve([ 'High = 89#Low = 28#Medium = 77#Low = 23', '1250' ])